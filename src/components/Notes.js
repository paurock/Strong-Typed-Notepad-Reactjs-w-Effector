...
import { v4 } from 'uuid';
import { $input, $notes, addNote, $techVars, updateTechVars, onRemove, editNote, getInputText } from '../model';
...
export const AddNoteForm = ({ name }) => {
	...
	const { noteUnderEdit } = useStore($techVars);
	const handleChange = e => getInputText(e.target.value);
	const handleSubmit = e => {
		e.preventDefault();
		input !== '' ? addNote({ note: input, id: v4(), category: name }) : alert('Please enter something!');
	};
	const onCancel = e => {
		updateTechVars({ noteUnderEdit: false });
		handleSubmit(e);
		console.log('cancel');
	};
	return (
...
const FormItem = ({ input, handleSubmit = f => f, handleChange = f => f, noteUnderEdit, onCancel = f => f }) => (
	<div className="form">
...
export const NoteList = ({ name }) => {
	...
	const handleEditNote = noteObj => {
		updateTechVars({ noteUnderEdit: true });
		editNote(noteObj);
	};
			...
			{filteredNotes.length !== 0 ? (
				filteredNotes.map(noteObj => (
					<div className="note" key={v4()}>
						<span className="timestamp"> <Ddmmyyyy/> </span>
						<span className="close" key={v4()} onClick={() => onRemove(noteObj.id)}>
							x
						</span>
						<div className="note-text" key={v4()} onClick={() => handleEditNote(noteObj)}>
							{noteObj.note}
						</div>
					</div>
				))
